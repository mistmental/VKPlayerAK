...
@interface MusicPlayerViewController ()  <UIPopoverControllerDelegate>
...
- (void) createPopover {
    ...
    [self createPopoverButtons];
    

}

- (void) createPopoverButtons {
    if (self.addButton == nil) {
        
        self.addButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
        self.addButton.frame = CGRectMake(0, 0, self.popView.bounds.size.width/2, self.popView.bounds.size.height/3);
        self.addButton.center = CGPointMake(self.popView.bounds.size.width/2, self.popView.bounds.size.height*1/4);
        
        [self.addButton setTitle:[NSString stringWithFormat:@"add"] forState:UIControlStateNormal];
        [self.addButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [self.addButton.titleLabel setFont:[UIFont fontWithName:@"Palatino" size:self.view.bounds.size.height/32]];
        [self.addButton addTarget:self action:@selector(addMusicToPlaylist) forControlEvents:UIControlEventTouchUpInside];
        [self.popView addSubview:self.addButton];

    }
    if (self.deleteButton == nil) {
        
        self.deleteButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
        self.deleteButton.frame = CGRectMake(0, 0, self.popView.bounds.size.width/2, self.popView.bounds.size.height/3);
        self.deleteButton.center = CGPointMake(self.popView.bounds.size.width/2, self.popView.bounds.size.height*2/4);
        
        [self.deleteButton setTitle:[NSString stringWithFormat:@"delete"] forState:UIControlStateNormal];
        [self.deleteButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [self.deleteButton.titleLabel setFont:[UIFont fontWithName:@"Palatino" size:self.view.bounds.size.height/32]];
        [self.deleteButton addTarget:self action:@selector(deleteMusicFromPlaylist) forControlEvents:UIControlEventTouchUpInside];
        [self.popView addSubview:self.deleteButton];
        
    }
    if (self.saveButton == nil) {
        
        self.saveButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
        self.saveButton.frame = CGRectMake(0, 0, self.popView.bounds.size.width/2, self.popView.bounds.size.height/3);
        self.saveButton.center = CGPointMake(self.popView.bounds.size.width/2, self.popView.bounds.size.height*3/4);
        
        [self.saveButton setTitle:[NSString stringWithFormat:@"save"] forState:UIControlStateNormal];
        [self.saveButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [self.saveButton.titleLabel setFont:[UIFont fontWithName:@"Palatino" size:self.view.bounds.size.height/32]];
        [self.saveButton addTarget:self action:@selector(saveMusicToPlaylist) forControlEvents:UIControlEventTouchUpInside];
        [self.popView addSubview:self.saveButton];
...
- (void) addMusicToPlaylist {
    ...
    [[ServerManager sharedManager] addMusicToPlaylist:self.audioID andOwnerID:self.ownerID ifSucces:^{
        NSLog(@"done");
...
- (void) deleteMusicFromPlaylist {
    [[ServerManager sharedManager] deleteMusicToPlaylist:self.audioID andOwnerID:self.ownerID ifSucces:^{
        NSLog(@"done");
    } andIfFailure:^(NSError *error) {
         NSLog(@"%@", error.localizedDescription);
    }];
...
- (void) saveMusicToPlaylist {

   
    [[ServerManager sharedManager] downloadMusicToCacheWithURLString:self.urlToSong toPathWithName:[NSString stringWithFormat:@"%@-%@.mp3",self.musicArtist,self.musicTitle]];
